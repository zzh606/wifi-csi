...
def sage_generate_steering_vector(sage_const):
    ...
    tof_candidates = cp.zeros([F, np.size(tof_range, 1)], dtype = complex)
    aoa_candidates = cp.zeros([A, np.size(aoa_range, 1)], dtype = complex)
    doppler_candidates = cp.zeros([T, np.size(doppler_range, 1)], dtype = complex)

    temp1 = -1j * 2 * cp.pi * frequency_interval * cp.arange(0, F).reshape([F,1]) * tof_range
    for ii in range(0, len(temp1)):
        for jj in range(0, len(temp1[0])):
            tof_candidates[ii, jj] = (cp.exp(temp1[ii,jj]))  # 复数指数用cmath库
    sage_const.update({'tof_candidates': tof_candidates})

    cos_temp = cp.zeros([1, np.size(aoa_range, 1)])
    for ii in range(np.size(aoa_range, 1)):
        cos_temp[0, ii] = np.cos(aoa_range[0, ii])
    temp2 = -1j * 2 * cp.pi * antenna_spatio * cp.arange(0, A).reshape([A, 1]) * cos_temp
    for ii in range(0, len(temp2)):
        for jj in range(0, len(temp2[0])):
            aoa_candidates[ii, jj] = (cp.exp(temp2[ii, jj]))  # 复数指数用cmath库,或np.exp([...])
    sage_const.update({'aoa_candidates': aoa_candidates})

    temp3 = 1j * 2 * cp.pi * time_interval * cp.arange(0, T).reshape([T, 1]) * doppler_range
    for ii in range(0, len(temp3)):
        for jj in range(len(temp3[0])):
            doppler_candidates[ii, jj] = (cp.exp(temp3[ii, jj]))  # 复数指数用cmath库
...
def sage_signal(latent_parameter, latent_index, sage_const):
    ...
    tof_matrix_temp = cp.zeros([F, T, A], dtype=complex)
    t3 = sage_const['tof_candidates'][:, latent_index[0]]
    for kk in range(cp.size(tof_matrix_temp, 2)):
        for jj in range(cp.size(tof_matrix_temp, 1)):
            tof_matrix_temp[:, jj, kk] = t3
    tof_matrix = cp.transpose(tof_matrix_temp, [1, 0, 2])

    aoa_matrix_temp = cp.zeros([A, T, F], dtype=complex)
    t1 = sage_const['aoa_candidates'][:, latent_index[1]]
    for kk in range(cp.size(aoa_matrix_temp, 2)):
        for jj in range(cp.size(aoa_matrix_temp, 1)):
            aoa_matrix_temp[:, jj, kk] = t1
    aoa_matrix = cp.transpose(aoa_matrix_temp, [1, 2, 0])

    doppler_matrix = cp.zeros([T, F, A], dtype=complex)
    t2 = sage_const['doppler_candidates'][:, latent_index[2]]
    for kk in range(cp.size(doppler_matrix, 2)):
        for jj in range(cp.size(doppler_matrix, 1)):
            doppler_matrix[:, jj, kk] = t2

    latent_signal = cp.multiply(cp.multiply(cp.multiply(latent_parameter[3], tof_matrix), aoa_matrix), doppler_matrix)
    return latent_signal


def sage_expectation(csi_signal, latent_signal, expect_index, update_ratio):
    noise_signal = csi_signal - cp.sum(latent_signal, 3)
...
def sage_maximization(latent_signal, latent_parameter, latent_index, sage_const):
    ...
    for kk in range(cp.size(aoa_matrix_temp, 2)):
        for jj in range(cp.size(aoa_matrix_temp, 1)):
            aoa_matrix_temp[:, jj, kk] = sage_const['aoa_candidates'][:, latent_index_temp[1]]
    aoa_matrix = cp.transpose(aoa_matrix_temp, [1, 2, 0])
    ...
    for kk in range(cp.size(doppler_matrix, 2)):
        for jj in range(cp.size(doppler_matrix, 1)):
            doppler_matrix[:, jj, kk] = sage_const['doppler_candidates'][:, latent_index_temp[2]]

    coeff_matrix = cp.multiply(cp.multiply(latent_signal, cp.conj(aoa_matrix)), cp.conj(doppler_matrix))

    coeff_vector = cp.reshape(cp.sum(cp.sum(coeff_matrix, 0), 1), [F, 1], order='F')
    coeff_vector = cp.tile(coeff_vector, [1, len(tof_range)])
    object_vector = cp.abs(cp.sum(cp.multiply(coeff_vector, cp.conj(sage_const['tof_candidates'])), 0))
    latent_index_temp[0] = int(cp.where(object_vector == cp.max(object_vector))[0])
    ...
    for kk in range(cp.size(tof_matrix_temp, 2)):
        for jj in range(cp.size(tof_matrix_temp, 1)):
            tof_matrix_temp[:, jj, kk] = sage_const['tof_candidates'][:, latent_index_temp[0]]
    tof_matrix = cp.transpose(tof_matrix_temp, [1, 0, 2])

    coeff_matrix = cp.multiply(cp.multiply(latent_signal, cp.conj(doppler_matrix)), cp.conj(tof_matrix))
    coeff_vector = cp.reshape(cp.sum(cp.sum(coeff_matrix, 0), 0), [A, 1], order='F')
    coeff_vector = cp.tile(coeff_vector, [1, len(aoa_range)])
    object_vector = cp.abs(cp.sum(cp.multiply(coeff_vector, cp.conj(sage_const['aoa_candidates'])), 0))
    latent_index_temp[1] = int(cp.where(object_vector == cp.max(object_vector))[0])
    ...
    coeff_matrix = cp.multiply(cp.multiply(latent_signal, cp.conj(aoa_matrix)), cp.conj(tof_matrix))
    coeff_vector = cp.reshape(cp.sum(cp.sum(coeff_matrix, 1), 1), [T, 1], order='F')
    coeff_vector = cp.tile(coeff_vector, [1, len(doppler_range)])
    object_vector = cp.abs(cp.sum(cp.multiply(coeff_vector, cp.conj(sage_const['doppler_candidates'])), 0))
    latent_index_temp[2] = int(cp.where(object_vector == cp.max(object_vector))[0])
    ...
    coeff_matrix = cp.multiply(cp.multiply(cp.multiply(cp.conj(aoa_matrix), cp.conj(tof_matrix)), cp.conj(doppler_matrix)), latent_signal)
    latent_parameter_temp[3] = cp.sum(coeff_matrix) / (T * F * A)
...
def sage_sfg(csi_signal, initial_parameter, initial_index, sage_const, threshold, qualified_num, bigger_num):
    ...
    for ii in range(N):
        ...
        for jj in range(L):
            ...
            temp_parameter[:, jj],temp_index[:, jj] = sage_maximization(temp_signal, final_parameter[:, jj], final_index[:, jj], sage_const) #正确,numpy 0.004s
            t3 = time.time()
        ...
        # if cp.sum(threshold >= parameter_diff) >= qualified_num or cp.sum(parameter_diff_temp < parameter_diff) >= bigger_num:
        #     break
        if parameter_diff[0] < 1e-9 and parameter_diff[1] < 1 / 180 * np.pi and parameter_diff[2] < 1 and parameter_diff[3] < 1e-9:
            break

    residue_error = csi_signal - cp.sum(latent_signal, 3)
    residue_error = cp.mean(cp.abs(residue_error)) / cp.mean(np.abs(csi_signal))
...
def sage_main(csi_data, G, sage_const, exact, qualified_num, bigger_num):
    ...
    estimated_parameter = cp.zeros([4, L, G], dtype=complex)
    initial_parameter = cp.zeros([4, L], dtype=complex)
    initial_index = cp.zeros([3, L], dtype=int)
    initial_index[0, :] = int(cp.floor((0 - sage_const['tof_range'][0, 0]) / (sage_const['tof_range'][0, 1] - sage_const['tof_range'][0, 0])))
    initial_index[1, :] = int(cp.floor((0 - sage_const['aoa_range'][0, 0]) / (sage_const['aoa_range'][0, 1] - sage_const['aoa_range'][0, 0])))
    initial_index[2, :] = int(cp.floor((0 - sage_const['doppler_range'][0, 0]) / (sage_const['doppler_range'][0, 1] - sage_const['doppler_range'][0, 0])))
    residue_errors = cp.zeros([1, G], dtype=complex)

    print('SAGE Main...')
    for ii in range(G):
        print(ii + 1)
        csi_signal = cp.transpose(csi_data[GI[ii] + cp.arange(0, T), :])
        csi_signal = cp.transpose(cp.reshape(csi_signal, [F, A, T], order='F'), [2, 0, 1])
...
def dynamic_analize(csi_data, time_stamp, count, step, orient, pre_buffer):
    ...
    TI = 1 / sample_rate
    AS = 0.5
    TR = cp.arange(-100, 401, 1).reshape([1, 501]) * 1e-9
    AR = cp.arange(0, 181, 1).reshape([1, 181]) / 180 * cp.pi
    DR = cp.arange(-80, 81, 1).reshape([1, 161])
    UR = 1
    ...
    MN = 5
    MS = 2
    parameter_weight = cp.array([1e9, 180 / np.pi, 1, 1]).reshape([1, 4]) * cp.array(
        [1 / 200, 1 / 90, 1 / 80, 10]).reshape([1, 4])

    csi_data_cut = 50
    csi_data_len = np.size(csi_data, 0)
    ...
    time_stamp_ex = np.concatenate((pre_time_stamp, time_stamp))
    pre_time_stamp = time_stamp[-pre_time_stamp_len:]
    # time_stamp_diff = list(np.diff(time_stamp_ex, axis=0))  # 没有用到
    # time_wrap_index = [time_stamp_diff.index(ii) for ii in time_stamp_diff if ii < 0]  # 没有用到
    ...
    ###############################################################################################################################
    t1 = time.time()
    estimated_parameter = sage_main(cp.asarray(csi_interp), 1000, sage_const, [1e-9, 1 / 180 * np.pi, 1, 1e-9], 4, 4)
    print('SAGE用时：', time.time() - t1)
    print('开始匹配路径')
    t1 = time.time()
    estimated_path = sage_path_mapping_filter(estimated_parameter, parameter_weight, MN, MS)
