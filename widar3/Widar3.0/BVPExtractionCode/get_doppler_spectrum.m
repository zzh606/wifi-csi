...
for ii = 1:rx_cnt
    ...
    csi_data = csi_data(round(1:1:size(csi_data,1)),:);  % �˴�δ������
rx_cnt
...
rx_cnt
    ...
    % PCA analysis.
    [pca_coef, pca_score] = pca(conj_mult);
    conj_mult_pca = conj_mult * pca_coef(:,1);
    
%     [S,F,T,P] = spectrogram(conj_mult_pca, 256, 255,255,1000);
%     figure;
%     colormap(jet);
%     mesh(F,T,circshift(abs(S)',[floor(size(S)/2) 0]));
%     view([90,90])
%     pause
...
%   [tfr,t,freq] = tfrsp(abs(conj_mult_pca), time_instance,...
%                 samp_rate); 
%             freq(1:5)
%             freq(end-5:end)
%             tfr(1:5)
...
%     pause
%     colormap(jet);
%     mesh(1:size(tfr,2),-size(tfr,1)/2:size(tfr,1)/2-1,squeeze(tfr));
%     pause
        end
    end
...
% plot(1:size(freq_time_prof,1),freq_time_prof(:,1)')
% pause
